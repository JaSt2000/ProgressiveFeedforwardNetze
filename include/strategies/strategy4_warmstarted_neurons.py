...
def warmstarted_neurons_expand(device, model, num_total_expansions, model_warmstarted, num_expansions):
    ...
    if torch.numel(
            indices) > 0:  # there are still edges (and hence neurons) to be added  (alternatively ---> identify abs_val of a neuron as sum of abs edge vals)
        initialized_neurons_indices = []
        ...
        for elem in indices:
            if len(initialized_neurons_indices) == num_neurons_to_be_added:
                break
            ...
            neuron_index = mm.get_global_index_neuron_array(layer_index, row_index, model.weight_mask_list)
            if neuron_index not in initialized_neurons_indices:
                mm.initNeuron(device, model, layer_index, row_index)
                initialized_neurons_indices.append(neuron_index)
    return flag_actual_expansion
