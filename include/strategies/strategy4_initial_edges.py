...
def initial_edges_expand(device, model, num_total_expansions, model_initial, num_expansions):
    ...
    result_abs_list_from_initial_model = (-1) * torch.ones(size=(num_params_total,)).to(torch.float32).to(
        device)
    get_abs_edges_array(model_initial, model.weight_mask_list, model.bias_mask_list,
